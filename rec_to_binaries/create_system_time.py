...
from read_binaries import (readTrodesExtractedDataFile,
...
def create_and_add_system_timepoints(cont_time):
	...
	if 'timestamp_at_creation' in cont_time:
		interval = (cont_time['data'][0][0] -
		            int(cont_time['timestamp_at_creation'])) * nanosec_per_sample
	else:
		interval = (cont_time['data'][0][0] -
		            int(cont_time['Timestamp_at_creation'])) * nanosec_per_sample
	...
	if 'system_time_at_creation' in cont_time:
		sys_time[0] = (
		    interval + (int(cont_time['system_time_at_creation']) * (1e6))).astype(int)
	else:
		sys_time[0] = (
		    interval + (int(cont_time['System_time_at_creation']) * (1e6))).astype(int)
	...
	for i in range(1, np.size(sys_time)):
	    ...
	    if (trodes_interval % 3 == 0):
	        sys_interval = int(trodes_interval *
	                           nanosec_per_sample) + int(trodes_interval / 3)
	    elif (i % 3 == 0):
